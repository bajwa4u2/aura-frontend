...
class RealtimeMediaService {
  ...
  /// controller-level certification harness. Returns the live object rather
  /// than a copy so signalling state, senders, receivers and transceiver
  /// directions can be READ; nothing here mutates. Never called by product
  /// code — it exists so a harness can verify the negotiation invariants
  /// independently of the code that implements them.
  @visibleForTesting

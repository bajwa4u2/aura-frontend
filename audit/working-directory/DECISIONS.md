...
## 2026-08-14: Domain 9 — Frontend Contextual Mention Eligibility, this repo's half of the founder-approved final pre-commit closure; commit still held (aura-backend-owned gate) pending report review.

Canonical record (aura-backend-owned): `../aura-backend/capability/FOUNDER_ACCEPTANCE_REGISTER.md` Domain 9, "Frontend Contextual Mention Eligibility — Part A" subsection. Founder approved the backend authority AND the institution-eligibility interpretation as FROZEN (Space-owned Thread → owning institution only; Personal Space → none; DM → only a participant that IS an institution) — do not revisit without a new founder decision. New `lib/core/tagging/mention_scope.dart` (`MentionScope.global()`/`.bounded(...)`) gives `GovernedTagAutocomplete` an optional `mentionScope` param (default `global()`, so Posts/Institution Posts/Announcements/replies are unchanged with zero edits to those 4 call sites). New `lib/core/tagging/mention_scope_providers.dart` derives bounded candidate sets for Thread/Space (`threadMentionScopeProvider`) and DM (`directThreadMentionScope`) from data these screens' existing endpoints already return — no new backend endpoint. **If you're adding an 8th `GovernedTagAutocomplete` call site on a bounded (Thread/Space/DM-like) surface, pass a `MentionScope.bounded(...)` — do not leave it on the `global()` default, which would re-open the exact "UI offers a target the backend then rejects" defect this checkpoint closed.** 11 new tests (`mention_scope_providers_test.dart`, `mention_scope_autocomplete_test.dart`), full 401/401 suite + `flutter analyze` clean.
...
## 2026-08-14: Release-Client Product-Reality Reconciliation — Layered Certification Governance + Founder Acceptance Register adopted (aura-backend-owned doctrine, this repo's changes reviewed under it), checkpoint APPROVED FOR COMMIT.
...
Founder authorized Phase 1 (Backend Canonical Lifecycle / Participant Truth) with a backend-only scope, then conditionally approved it pending a guest-disconnect investigation and a disposable-Postgres migration replay — both resolved same day, both entirely within `aura-backend`. `aura-backend` committed `214e48e` and pushed to `origin/main` — Gate 2 APPROVED, PHASE 1 CLOSED. This repo's canonical Dart mirror (`lib/core/realtime_canonical/`) required no change at any point in this chapter since the contracts themselves were never touched. See `aura-backend/DECISIONS.md`'s matching 2026-08-14 entries for the full decision record.

...
Last updated: 2026-08-14 UTC (URGENT: Meetings router regression restored — a booked/invited attendee with no institution access was redirected to Institution Sign In; fixed at the router-classification shared boundary, zero Meetings-file changes; commit pending. Permanent Call Lifecycle Reconciliation from earlier the same session also present below; founder device certification for both still PENDING)

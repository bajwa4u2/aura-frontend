...
## Institution Ownership Continuity — COMPLETION PASS DELIVERED, 2026-08-14

Completion pass, all four founder decisions resolved. This repo's half: a governed platform-admin OWNERSHIP RECOVERY surface in the EXISTING admin institution members screen (`admin_institution_members_screen.dart`) — recovery-required banner shown ONLY when the backend reports no actionable owner, replacement chosen from backend-supplied eligible candidates (never assembled client-side), explicit reason required, confirmation naming institution/prior owner/replacement/privileged nature; `admin_repository.dart` gained `fetchOwnershipRecoveryState`/`emergencyRecoverOwnership`, `admin_models.dart` gained `InstitutionOwnershipRecoveryState`/`OwnershipRecoveryCandidate`; `notification_presentation.dart` gained the `INSTITUTION_OWNERSHIP_RECOVERED` governance phrase. **Two real pre-existing defects found by the new widget test and fixed**: (1) a generic "Promote to Owner" control the backend has ALWAYS refused (`updateMemberRole` rejects `role: OWNER`) — a guaranteed-error dead control that also contradicted the doctrine that ownership is never an ordinary role assignment; (2) the screen's blocking progress overlay was only cleared in `catch`, so it stayed up permanently after every SUCCESSFUL role change or member removal — fixed on all three action paths. **Reported, NOT fixed**: a phantom `EDITOR` institution role across this repo (notably selectable in `invite_create_screen.dart`) though the backend enum has only OWNER/ADMIN/MEMBER — needs its own decision. 6 new widget tests mounting the REAL admin screen; 411/411 tests, `flutter analyze` clean.

## Institution Ownership Continuity — this repo's half implemented, NOT yet committed, 2026-08-14 (superseded above)

Canonical record (aura-backend-owned): `../aura-backend/capability/INSTITUTION_OWNERSHIP_CONTINUITY_DOCTRINE.md`. This repo's changes: `lib/screens/account_deletion_screen.dart` now surfaces the backend's `OWNERSHIP_TRANSFER_REQUIRED` error truthfully (names the affected institution(s), tells the user to transfer ownership first) instead of a generic failure message; `institutions_repository.dart` gained `emergencyRecoverOwnership(...)`, making the platform-emergency-recovery capability callable. No dedicated platform-admin recovery dashboard/picker UI was built this pass — deliberately out of scope (break-glass tooling), recommended as a follow-up only if founder visibility needs exceed direct API operation. `flutter analyze` clean, full suite passing.

## Account Lifecycle / Public Identity — this repo's half CLOSED, committed and pushed, 2026-08-14 (`bb5c3c0`, paired aura-backend `9ecc72c`)

Canonical record (aura-backend-owned): `../aura-backend/capability/ACCOUNT_LIFECYCLE_DOCTRINE.md` and `capability/FOUNDER_ACCEPTANCE_REGISTER.md`'s "Account Lifecycle / Public Identity" section. This repo's changes: `Profile` domain model gained `accountStatus`/`isActive` (`lib/features/profile/domain/profile.dart`); `AuthorProfileScreen`'s Follow/Message affordances now also require `profile.isActive`, and a truthful (never judgmental, never a moderation reason) inactive/deleted notice renders in their place. `GET /search`'s result set now excludes non-ACTIVE users backend-side — no frontend change needed there, the existing global-search consumers (mention autocomplete, `NewConversationScreen`, realtime-room add-participant) all inherit it automatically. 1 new test (`profile_account_status_test.dart`), `flutter analyze` clean, 402/402 suite.

## Domain 9 — CLOSED, committed and pushed, 2026-08-14 (`88efffb`, paired aura-backend `4eb214f`)

Canonical record: `../aura-backend/capability/FOUNDER_ACCEPTANCE_REGISTER.md` Domain 9's CLOSEOUT block. **Active next item (aura-backend-owned): Part B — `GET /search` unauthenticated-access security investigation, IN PROGRESS.** Do not begin Item 17 before that's reviewed.

## Domain 9 — Frontend Contextual Mention Eligibility, 2026-08-14 — corrected, commit held pending report review

Canonical record (aura-backend-owned): `../aura-backend/capability/FOUNDER_ACCEPTANCE_REGISTER.md` Domain 9, "Frontend Contextual Mention Eligibility — Part A". `GovernedTagAutocomplete` gained an optional `mentionScope` param (new `lib/core/tagging/mention_scope.dart`, default `MentionScope.global()` — zero behavior change for Posts/Institution Posts/Announcements/replies). Thread/Space and DM composers now pass `MentionScope.bounded([...])`, derived by new `lib/core/tagging/mention_scope_providers.dart` from data these screens' existing endpoints already return (Thread/Space membership + owning institution; a DM's own other participant) — no new backend endpoint, local substring filtering, zero network call for bounded surfaces. Closes both directions of Domain 9's DM privacy-leak/Thread dead-mention pair at the frontend layer (backend already closed them independently). 11 new tests, full 401/401 suite + `flutter analyze` clean.

## Domain 13 — Conversation Lifecycle / Retention Consequence Integrity, 2026-08-13 — COMPLETE reversible lifecycle (archive + restore + discoverability), ARCHITECTURE/IMPLEMENTATION PASS

Founder held commit on the first pass (identity/personal-archive wiring only) pending complete archive+restore+discoverability for every surface. Full record (aura-backend-owned): `../aura-backend/capability/FOUNDER_ACCEPTANCE_REGISTER.md` Domain 13. This repo's completion pieces, new this pass: `ArchivedSpacesScreen` (`/me/correspondence/archived`, linked from the Messages hub header "Archived" chip) with per-row Restore; `ArchivedThreadsScreen` (`/me/correspondence/:spaceId/archived-threads` and the institution equivalent, linked from the Threads tab) showing "Archived for you"/"Archived for everyone" independently with the matching restore action(s) per row — a Thread archived both ways shows both; `InstitutionSpacesScreen` gained an admin-only "View archived spaces" toggle + Restore. `flutter analyze` clean, 390/390 practical suite (unchanged — no new widget tests, backend authority is the tested layer).

## Domain 13 (superseded above) — Conversation Lifecycle / Retention Consequence Integrity, 2026-08-13 — frontend wiring, ARCHITECTURE/IMPLEMENTATION PASS

Full record (aura-backend-owned): `../aura-backend/capability/FOUNDER_ACCEPTANCE_REGISTER.md` Domain 13 section. This repo's changes: `InboxScreen(archived:)` + swipe-to-archive/unarchive + new `/messages/direct/archived` route (DM personal archive); `space_screen.dart`'s "Archive space" now branches on `institutionId` (institutional → `archiveInstitutionSpace`, personal → new `setPersonalArchived`) with corrected per-case dialog copy; `thread_screen.dart` gained a genuinely separate "Archive for me" (personal, any member) menu item alongside the existing "Archive conversation (everyone)" (global, owner/admin/editor) — the two call different repository methods and never merge. `flutter analyze` clean, 390/390 practical suite. Disclosed gap: no archived-list browse UI built for Personal Spaces or Threads yet (DMs only).

## Mandatory Discovery & Improvement Reporting Governance FROZEN, 2026-08-13 — platform-wide (aura-backend-owned, this repo references)

Canonical: `../aura-backend/capability/MANDATORY_DISCOVERY_AND_IMPROVEMENT_REPORTING_GOVERNANCE.md`. Core rule: discovery creates a reporting obligation, not automatic implementation authority — any material defect, gap, or meaningful refinement opportunity encountered while working in this repo must be reported/classified/recommended, never silently dropped as "outside scope." Every future checkpoint closeout in this repo must include a "Discoveries Outside the Direct Implementation" section (stated even when empty). Full findings from this checkpoint (CORS/429 boot-cause, `addInstitutionSpaceMember` missing notification, a CRLF-file tooling gotcha) recorded in aura-backend's `capability/FOUNDER_ACCEPTANCE_REGISTER.md` — not duplicated here.

## Release-Client Product-Reality Reconciliation — Founder Acceptance Register adopted, 2026-08-14 — FOUNDER-APPROVED CHECKPOINT, committed

Canonical doctrine/register (aura-backend-owned, this repo references not duplicates): `../aura-backend/capability/LAYERED_CERTIFICATION_GOVERNANCE.md`, `../aura-backend/capability/FOUNDER_ACCEPTANCE_REGISTER.md`, `../aura-backend/capability/INSTITUTION_SPACE_MEMBERSHIP_DOCTRINE.md`.

**This repo's changes, all founder-approved at the certification levels demonstrated (no domain described as fully closed while a higher layer remains OPEN):**

**Domain 6 — Cross-System Institutional Identity Coherence**: `router.dart`'s Institution Space route now passes `institutionId` into `SpaceScreen`; new `institutionsRepositoryProvider.getInstitutionSpace()`; `space_screen.dart` branches between the institution-aware and personal space-detail providers so the header renders real institution identity (avatar/name) instead of the wrong personal-space endpoint's response. ARCHITECTURE/IMPLEMENTATION PASS, PRODUCT-BEHAVIOR OPEN.

**Domain 7 — Membership Consequence Integrity**: new `institutionsRepositoryProvider.addInstitutionSpaceMember()`, new `_AddInstitutionMemberDialog` widget in `space_screen.dart` (reads eligible institution members via `listMembers()`, correctly nested under `member['user'][...]`) — a genuinely separate "Add member" action from "Invite," per the frozen backend doctrine, never solved by relabeling. ARCHITECTURE/IMPLEMENTATION PASS, PRODUCT-BEHAVIOR OPEN.

**Domain 8 — Invitation Lifecycle Integrity**: `institution_invites_screen.dart` now surfaces a truthful per-invite warning when `emailDeliveryStatus == 'FAILED'` ("Invite email could not be delivered... the code above still works") instead of implying successful delivery when the backend knows it failed. ARCHITECTURE/IMPLEMENTATION PASS, PRODUCT-BEHAVIOR OPEN, REAL-BOUNDARY OPEN.

**Domain 5 — Runtime Lifecycle / Attention Coherence**: founder-observed "You have an active call" banner reappearing on every browser/app open, traced to `OrphanedSessionBanner`'s dismissal set living only in ephemeral `State` — never persisted, so a still-genuinely-active session (correctly) resurfaced every reload/relaunch. New `lib/features/realtime/data/orphaned_session_dismissal_cache.dart` (`OrphanedSessionDismissalCache`), `SharedPreferences`-backed, persisted per session id (a new call still banners normally), self-pruning against the live `mySessions` snapshot on every write. Wired into `OrphanedSessionBanner.initState` (load) and `onDismiss` (persist) — dismissal still never mutates canonical session truth. `findOrphanedActiveSession` itself was re-confirmed already correct (its 6 pure-function tests were not touched). 5 new cache tests. ARCHITECTURE/IMPLEMENTATION PASS, PRODUCT-BEHAVIOR OPEN — founder will verify live: active call → banner → dismiss → refresh → browser restart → remains dismissed; session ends → stale dismissal/session state does not recreate incorrect attention.

Certification: full frontend suite 390/390 (up from 385), `flutter analyze` clean. Backend-side CORS/429 middleware-order fix and the invitation delivery-truth fix are aura-backend-owned, recorded there. **Do NOT begin Item 17.**

## Realtime Architecture Correction — Phase 4 (Notification/Ringing Projection Migration) CLOSED, 2026-08-14 — founder-approved, committed `e344dbc`, pushed

Full record (canonical, aura-backend-owned): `../aura-backend/docs/2026-08-14-realtime-architecture-correction-phase4-implementation.md`. This repo's changes: new `lib/features/realtime/application/incoming_call_projection.dart` — the ONE shared projection authority consolidating two previously-independent, already-drifted duplicate classifier implementations (`incoming_live_overlay.dart` and `notification_bridge.dart` each carried their own copy of call-kind/session/terminal-state resolution) into one shared, tested module, plus a new session-scoped `IncomingCallPrecedenceGuard` (bounded tombstone set) closing a genuine gap: nothing previously prevented a late/reordered incoming-call delivery from resurrecting a ringing card after that session's terminal truth (accepted/declined/expired/cancelled/ended) had already arrived. `IncomingCallBridgeNotifier` (`incoming_call_bridge.dart`) now wires the guard into `addIncoming`/`removeBySession`/`remove`/`evictExpired`/`clear`, and both its socket listeners now route through `projectCallPresentationEvent` instead of hardcoded per-event-name branches. `RealtimeSocketService`'s event whitelist gained the canonical dot-case wire names (`invite.issued`, `participant.accepted/declined/expired/cancelled`, `session.ended`) — previously silently dropped by the explicit `socket.on(name, ...)` per-name registration even though the backend already dual-emitted them since Phase 2/3.

...
- **Mention attachment remediation (2026-07-21):** `GovernedTagAutocomplete` owns token replacement and canonical `TagReference` callbacks. Member/institution post composers submit selected member/institution mentions in a `mentions` array while the inserted text remains present.
...
- Latest remediation verification: `flutter analyze` clean. Targeted and full `flutter test` invocations hung in this Windows workspace before output; treat as an environment/tooling blocker until a normal runner completes.

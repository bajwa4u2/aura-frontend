...
void main() {
  ...
  group('the backend presents once per physical device', () {
    const authority = '../aura-backend/src/common/devices/multi-device-authority.ts';
    const fanout = '../aura-backend/src/communications/push/push-notification.service.ts';
    const fcm = '../aura-backend/src/communications/push/adapters/fcm-push.adapter.ts';
    const apns = '../aura-backend/src/communications/push/adapters/apns-push.adapter.ts';

    bool _backendPresent() => File(authority).existsSync();
  ...
  group('the fallback is decided by delivery outcome, not by registration', () {
    ...
    test('suppression is conditioned on the presenting send actually succeeding', () {
      if (!File(fanout).existsSync()) return;
      final source = _read(fanout);

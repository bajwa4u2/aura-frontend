...
## FOUNDER-OBSERVED CORRECTION (2026-08-16, post-deployment — AMENDS THIS CLOSEOUT)

Production founder observation amended the frozen destination tree. **Authenticated primaries are now FOUR: Home · Messages · Discover · Create** (governing test: primary navigation represents fundamental recurring human intentions). ME removed (personal depth behind identity/avatar chrome; /me preserved as destination). MEETINGS removed (an institutional domain — the never-executable /meetings primary this closeout certified was a real defect; no personal landing is manufactured). CREATE restored ("action, not destination" superseded; global + contextual creation are complementary). Certification gap acknowledged and closed: the route-integrity gate tested registry self-consistency, not registry → router executability — it now proves every authority-declared address executable. Full rulings: docs/DECISIONS.md "FOUNDER-OBSERVED NAVIGATION CORRECTION". The remainder of this closeout (Navigation Authority, DR4 architecture, acting-identity retirement, enforcement) stands.
